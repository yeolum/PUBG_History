...
import type { Tournament, Stage, Match, StageTeamStanding, MatchTeamResult } from '@/lib/types'
...
export default async function TournamentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const allStageIds = stagesList.map((st) => st.id)
              ...
              return (
                <div key={stage.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                  ...
                  {stageStandings.length > 0 && (
                    <div className="p-5">
                      <p className="text-xs font-medium text-gray-500 mb-1 uppercase tracking-wide">팀 순위</p>
                      <p className="text-xs text-gray-400 mb-3">순위점수 + 킬점수 = 총점수 | 1~8위: 10,6,5,4,3,2,1,1</p>
                        ...
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="text-xs text-gray-400 border-b border-gray-100">
                              ...
                              <th className="text-right pb-2">순위점수</th>
                              <th className="text-right pb-2">킬점수</th>
                              <th className="text-right pb-2 font-bold text-gray-600">총점수</th>
                            </tr>
                          </thead>
                          <tbody>
                            {stageStandings.map((s, i) => (
                              <tr key={s.team_id ?? s.team_name} className="border-b border-gray-50 last:border-0">
                                ...
                                <td className="py-1.5 text-right text-gray-500">{s.placement_points}</td>
