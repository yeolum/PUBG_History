...
export default async function CircuitPage({ params }: Props) {
  ...
  // RTT2: all queries that only need tournamentIds run in parallel
  const [stages, ttData, finalStandingsData, ttsRows, tpsRows, kcRows] = await Promise.all([
  ...
  // Build player stats early so we can kick off playerCurrentTeam fetch in parallel with matches
  ...
  // RTT4: allTeamResults (needs matchIds)
  const allTeamResults = await fetchInChunked<AnyRow>(
    (chunk) => supabase
  ...
  // Build pubg-tag index to avoid O(n*m) scan inside the champions loop
  const pubgTagIndex = new Map<string, string>() // key: `${tournamentId}:${teamId}` → pubg_team_name
  for (const r of allTeamResults) {
    if (!r.pubg_team_name || !r.team_id) continue
    const tid = matchToTournament.get(r.match_id as string)
    if (!tid) continue
    const k = `${tid}:${r.team_id}`
    if (!pubgTagIndex.has(k)) pubgTagIndex.set(k, r.pubg_team_name as string)
  }
  ...
  for (const tid of tournamentIds) {
    ...
    const pubgTag = pubgTagIndex.get(`${tid}:${standing.team_id}`)
    if (pubgTag) championPubgTagByTournament.set(tid, pubgTag)
  ...
  // Period-correct name + logo override for champions via team_aliases
  ...
  if (championTeamIds.length > 0) {
    ...
    const aliasLogo = new Map<string, string>()
    ...
    for (const c of champions) {
      ...
      const pubgTag = championPubgTagByTournament.get(c.tournamentId) ?? null
      const tagLower = pubgTag?.toLowerCase() ?? ''
